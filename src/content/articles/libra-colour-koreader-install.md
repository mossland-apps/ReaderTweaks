...
type: guide
order: 1
summary: Add KOReader to the Libra Colour by copying one package to the device. This keeps the stock Kobo reader intact and adds a KOReader icon to the home screen.
...
prerequisites:
  - text: "KOReader on the Kobo Libra Colour — what it adds and what it doesn't"
    href: /devices/kobo/libra-colour/koreader/
nextStep:
  text: More KOReader guides for the Libra Colour
  href: /devices/kobo/libra-colour/koreader/
related:
  - text: "KOReader won't launch on the Kobo Libra Colour"
    href: /devices/kobo/libra-colour/troubleshooting/koreader-wont-launch/
...
searchTerms:
  ...
  - add koreader
steps:
  - instruction: Update the Libra Colour to the current firmware first
    detail: >-
  ...
  - instruction: Download the right KOReader package for the Libra Colour
    detail: >-
      On the KOReader releases page, take the newest normal release (dated, such
      as v2026.07.1 — not a "nightly"). The Libra Colour is one of the 2024
      Kobos on a newer hardware platform, so it needs the file with **"kobov5"**
      in the name, such as "koreader-kobov5-v2026.07.1.zip" — not the plain
      "koreader-kobo" file, which is for older Kobos, and not the Kindle or
      Android builds.
    warning: >-
      Installing the plain "koreader-kobo" package on a Libra Colour, Clara BW or
      Clara Colour is the most common reason KOReader will not start on these
      models. Match the "kobov5" file to the device.
  - instruction: Connect the Libra Colour to your computer with the USB-C cable
    detail: >-
  ...
  - instruction: Wait for the Libra Colour to process the update
    detail: >-
      ...
      program called KFMon, which is what draws the KOReader icon and starts the
      app.
  - instruction: Open KOReader from the home screen
    ...
    tip: >-
      ...
      Libra Colour from Settings, then Device information. KFMon adds the tile on
      the next clean start-up.
---

This is the standard way to run KOReader on a Kobo: you copy one package to the
device over USB, and the Kobo's own software installs it on the next start-up.
Nothing is overwritten. The stock Kobo reader, your library and your settings
stay exactly as they were, and you switch between the two by opening or closing
KOReader.
...
[troubleshooting guide](/devices/kobo/libra-colour/troubleshooting/koreader-wont-launch/).
