...
type: guide
order: 1
summary: The KOReader-specific steps for a jailbroken entry Kindle — installing the hotfix, KUAL and the KOReader package — plus how to protect it from firmware updates.
...
whatYouNeed:
  - Basic Kindle, already jailbroken for its firmware version
  - A computer
  - A USB-C data cable
...
steps:
  - instruction: Confirm the Kindle is already jailbroken
    detail: >-
      ...
      MobileRead forums — it changes over time and per firmware. Check your
      firmware under Settings, then Device Options, then Device Info.
  - instruction: Put the Kindle in airplane mode and turn off automatic updates
    detail: >-
      ...
      the jailbreak, so keep Wi-Fi off except for brief, deliberate syncs.
      Installing the update-blocking hotfix in a later step makes this safer.
    warning: >-
  ...
  - instruction: Download the three packages on your computer
    detail: >-
      ...
      package and the "Kindle update-blocking hotfix" (sometimes called the
      update hotfix or "Prevent OTA"). Keep the zip files as they are for now.
  - instruction: Connect the Kindle and copy the packages across
    detail: >-
      ...
      as each package's own readme specifies. The KOReader package is usually
      placed in the root or a "koreader" folder created by its installer.
    tip: >-
  ...
  - instruction: Install the update hotfix from KUAL
    detail: >-
      ...
      downloading and applying firmware updates on its own, which is what protects
      the jailbreak going forward.
  - instruction: Install KOReader from KUAL
    detail: >-
      Still in KUAL, choose "Install or update KOReader", sometimes shown as
      "MRPI" (the package installer). It unpacks the KOReader package you copied
      over. When it finishes, back out of KUAL.
  - instruction: Launch KOReader
    detail: >-
      Open KUAL again and choose "KOReader" to start it. The first launch takes
      15 to 30 seconds. To leave KOReader, use its top menu and choose exit,
      which returns you to the Kindle interface.
---
...
mode. A Kindle that quietly updates itself is the usual reason a working KOReader
setup breaks.
