---
title: KOReader won't launch on the Kobo Sage
...
symptoms:
  ...
  - The Sage restarts when you tap KOReader.
beforeYouStart:
  - Check which firmware version the Sage is on — Settings, then Device information.
...
fixes:
  - title: Put the installer files where the Kobo can find them
    ...
    steps:
      - Connect the Sage and open the KOBOeReader drive on your computer.
      - >-
  ...
  - title: Reinstall KOReader over the top of itself
    ...
    steps:
      ...
      - >-
        Connect the Sage and copy the contents of the zip to the top level of the
        drive, choosing to merge folders and replace files when asked.
      - Eject safely, unplug, and let the device install.
      - After it finishes, restart the Sage once from Settings, then Device information.
    note: >-
  ...
  - title: Rule out a firmware change or a stuck launcher
    ...
    steps:
      ...
      - >-
        ...
        from Settings first, then try again.
      - >-
...
recovery: >-
  ...
  launcher files. To remove it and put the Sage back as it was: connect the
...
related:
  ...
  - text: "KOReader on the Kobo Sage"
    href: /devices/kobo/sage/koreader/
  - text: "The Kobo Sage won't turn on or is frozen"
    href: /devices/kobo/sage/troubleshooting/wont-turn-on/
---

If KOReader worked before and stopped after a Kobo update, start with Fix 3. If
it has never opened since you installed it, work through the fixes in order — Fix
1 resolves the large majority of first-time installs.
