---
title: The BOOX Page battery drains fast
pageSlug: battery-drains-fast
device: boox-page
software: BOOX firmware
summary: >-
  The Page loses charge quickly even when idle. On an Android e-reader this is
  almost always background apps, sync, or Wi-Fi left on.
difficulty: Easy
...
fixes:
  - title: Restrict background apps and manage Wi-Fi
    ...
    steps:
      - >-
        ...
        list. Set apps you use only occasionally to "optimised" or "restricted"
        so Android can close them when not in use.
      ...
      - In each reading app's own settings, turn off "sync on open" or set the sync interval to manual.
  - title: Find an app that is holding the device awake
    ...
    steps:
      - >-
        ...
        usage since the last full charge. An app near the top that you have barely
        opened is the suspect.
  ...
  - title: Check optimisation and auto-sync settings
    ...
    steps:
      - Open Settings, then "Accounts", and turn off auto-sync for any Google or other account you do not need syncing continuously.
      ...
      - As a last resort, back up sideloaded books and do a factory reset to clear a system-level problem, then add apps back carefully.
recovery: >-
...
related:
  ...
  - text: "The BOOX Page won't turn on or is frozen"
    href: /devices/boox/boox-page/troubleshooting/wont-turn-on/
---

Start by turning Wi-Fi off between syncs and restricting background apps — that
alone brings most Pages back to many days of standby. If it is still bad, the
