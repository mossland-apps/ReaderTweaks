---
title: A DRM-protected library book won't open on the PocketBook InkPad Color 3
...
summary: >-
  ...
  InkPad Color 3 signed in to an Adobe ID.
difficulty: Moderate
...
symptoms:
  ...
  - A library book opened on another device but not on the InkPad Color 3.
beforeYouStart:
...
mostCommonCause: >-
  The InkPad Color 3 can open Adobe-DRM library books, but only once it is
  authorised with an Adobe ID, and each Adobe ID can authorise a limited number
  of devices. The usual causes are: the device was never signed in to Adobe, the
  book was fulfilled to a different Adobe ID than the one on the device, or the
  ".acsm" file was copied over instead of being opened on the device to complete
  the download.
decisionTree:
  ...
  branches:
    ...
    - label: "Adobe sign-in itself fails with an activation limit"
      ...
      href: "#fix-2"
fixes:
  - title: Authorise the InkPad Color 3 with an Adobe ID
    rationale: >-
    ...
    steps:
      - >-
        On the InkPad Color 3, open Settings, then "Accounts and Synchronization",
        then "Adobe DRM".
      - >-
    ...
    note: >-
      ...
      any computer, and the InkPad Color 3 — so books unlock on all of them.
  - title: Make sure the book was fulfilled to the right Adobe ID
    rationale: >-
      ...
      one now on the device, the InkPad Color 3 cannot open it, and re-authorising
      to a new ID does not retroactively unlock older books.
    steps:
      - >-
      ...
      - >-
        Sign the InkPad Color 3 in to that same Adobe ID (Settings, then Accounts
        and Synchronization, then Adobe DRM). If the device is already signed in
        to a different ID, sign out first.
      - >-
  ...
  - title: Complete the download from the ".acsm" file on the device
    rationale: >-
      ...
      exchanges for the actual book. Copying the ".acsm" onto the InkPad Color 3
      and opening it there completes the download; copying a book fulfilled on a
    ...
    steps:
      ...
      - Copy just the ".acsm" file to the InkPad Color 3 by USB, into any folder.
      - >-
        On the device, open the ".acsm" from the Library. With Adobe DRM
        authorised (Fix 1), the InkPad Color 3 contacts Adobe, downloads the real
        EPUB or PDF, and replaces the ".acsm" entry with the book.
      - >-
        If it still fails, the cleanest route is to fulfil the loan in the Libby
        app and use its "Send to device" option, which hands the device a book
...
related:
  - text: "Set up a new PocketBook InkPad Color 3"
    href: /devices/pocketbook/pocketbook-inkpad-color-3/guides/first-setup/
  - text: "Get books onto the PocketBook InkPad Color 3"
    href: /devices/pocketbook/pocketbook-inkpad-color-3/guides/get-books-on/
---

The InkPad Color 3 opens Adobe-DRM library books without a computer, but only
after it is signed in to an Adobe ID — and the book has to have been issued to
that same ID. Fix 1 covers a device that was never authorised; Fix 2 covers a
mismatch between the book's ID and the device's.
