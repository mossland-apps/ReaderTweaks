---
title: The PocketBook Era won't turn on or is frozen
...
mostCommonCause: >-
  ...
  stopped responding, which holding the power button clears. On the Era, a large
  audiobook library being indexed can also make a start feel stuck.
decisionTree:
...
fixes:
  - title: Charge it, then force a restart
    ...
    steps:
      - >-
        Plug the Era into a wall charger with a known-good USB-C cable and leave
        it for at least 30 minutes, longer if you can. A charging light or icon
        may not appear straight away.
      - >-
  ...
  - title: Force a restart to clear a hang or a stuck logo
    ...
    steps:
      ...
      - >-
        ...
        same page afterwards. After copying on a lot of audiobooks, give the
        first start several minutes to index before deciding it is stuck.
      - >-
  ...
  - title: Repair the software if it keeps freezing
    rationale: >-
      Repeated freezes after a normal start usually trace to one damaged book or
      audio file or a corrupt library database, not the hardware.
    steps:
      - >-
        Connect the device to a computer. Remove the last one or two files you
        added before the trouble began, and see if the freezes stop.
      - >-
        If a specific book or audiobook crashes the reader every time it opens,
        delete that file and add a fresh copy.
      - >-
...
related:
  ...
  - text: "Bluetooth audio drops out or won't pair on the PocketBook Era"
    href: /devices/pocketbook/pocketbook-era/troubleshooting/bluetooth-audio-problems/
---

Follow the fixes in the order the decision tree points to. A long charge plus a
30-second power-button hold revives most unresponsive Eras; a stuck logo or
frozen page is nearly always cleared by the same hold on its own. Give a first
start after a big audiobook copy a few minutes before assuming it has hung.
