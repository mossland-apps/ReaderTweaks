---
title: Books bought or sent aren't showing up on the Kindle Paperwhite
pageSlug: books-not-showing
device: paperwhite
software: Kindle firmware
summary: >-
  A book you bought, borrowed or sent is missing from the Paperwhite. Usually
...
beforeYouStart:
  ...
  - Know which Amazon account the Kindle is registered to — check Settings, then Your Account.
...
fixes:
  - title: Sync the Kindle and check what it is showing you
    ...
    steps:
      - >-
        Open Settings and choose "Sync", or swipe down and tap the sync icon.
        Wait a minute.
      - >-
      ...
      - >-
        Check the sort order is not hiding the book. Set it to "Recent" and look
        again.
      - >-
  ...
  - title: Fix a Send to Kindle delivery that never arrived
    ...
    steps:
      - >-
        ...
        Preferences, then "Personal Document Settings". Check that the address
        you sent from is on the "Approved Personal Document E-mail List", and add
        it if not.
      - >-
        Confirm you sent the file to this Kindle's own "@kindle.com" address, not
        another device's, and that the file is under the current size limit
        (200 MB) and in a supported format.
      - >-
        Re-send the file. If you used email, put a word in the subject line so it
  ...
  - title: Sort out a library or borrowed book
    ...
    steps:
      - >-
        In the Libby or OverDrive app, open the loan and check it is still
        active and was "sent to Kindle" for this device, not another one.
      - >-
        On the Kindle, sync and set the Library filter to "All". A borrowed book
        behaves like a purchase once delivered.
      - >-
...
recovery: >-
  ...
  re-synced to the Paperwhite.
related:
  - text: "Kindle Paperwhite won't connect to Wi-Fi"
    href: /devices/kindle/paperwhite/troubleshooting/wifi-wont-connect/
  - text: "Get your own EPUB books onto the Kindle Paperwhite"
    href: /devices/kindle/paperwhite/guides/send-to-kindle/
---

If the decision tree does not point you somewhere, start with Fix 1 — a sync
plus the "All" filter resolves the majority of "missing book" reports on the
Paperwhite.
