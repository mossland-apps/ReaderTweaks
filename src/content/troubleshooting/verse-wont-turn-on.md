---
title: The PocketBook Verse won't turn on or is frozen
...
decisionTree:
  ...
  branches:
    ...
    - label: It starts fine but keeps freezing later
      hint: Likely a bad file, a full disk or a faulty card
      href: "#fix-3"
fixes:
  - title: Charge it, then force a restart
    ...
    steps:
      - >-
        Plug the Verse into a wall charger with a known-good USB-C cable and
  ...
  - title: Force a restart to clear a hang or a stuck logo
    ...
    steps:
      ...
      - >-
        If you have a microSD card in, power off, remove the card, and start
        again. A failing or badly formatted card can stall the boot. If it
        starts without the card, reformat the card (FAT32 or exFAT) before
        reusing it.
  - title: Repair the software if it keeps freezing
    rationale: >-
      Repeated freezes after a normal start usually trace to one damaged book
      file, a corrupt library database, or a bad microSD card — not the
      hardware.
    steps:
      - >-
...
recovery: >-
  ...
  account or cloud library, which can be re-synced after you sign in, and not
  anything on a microSD card. If the device shows no response after an hour of
  charging, a fresh cable and charger, and a 30-second power hold, it is likely a
  hardware fault and worth contacting PocketBook support.
related:
  - text: "Set up a new PocketBook Verse"
    href: /devices/pocketbook/pocketbook-verse/guides/first-setup/
  - text: "A DRM-protected library book won't open on the PocketBook Verse"
    href: /devices/pocketbook/pocketbook-verse/troubleshooting/drm-book-wont-open/
---

Follow the fixes in the order the decision tree points to. A long charge plus a
30-second power-button hold revives most unresponsive Verses; a stuck logo or
frozen page is nearly always cleared by the same hold on its own. If it keeps
freezing, suspect a bad book file or the microSD card.
