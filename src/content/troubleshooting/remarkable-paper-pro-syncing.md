---
title: Notebooks or documents aren't syncing on the reMarkable Paper Pro
pageSlug: syncing
...
mostCommonCause: >-
  Most often the device simply has not finished syncing — it needs Wi-Fi and time,
  and a large notebook with a lot of ink uploads slowly. Some cloud features and
...
fixes:
  - title: Force a sync and refresh the app
    rationale: >-
      ...
      off Wi-Fi, the notebook is still only on the Paper Pro.
    steps:
      - >-
        On the Paper Pro, make sure Wi-Fi is on. Open Settings and look for a
  ...
  - title: Fix a document that never reached the device
    rationale: >-
      ...
      the Paper Pro. Either leg can stall on a weak connection, and a corrupt or
      huge PDF can fail to process.
    steps:
      - >-
        Confirm the device and the app are signed in to the same reMarkable
        account.
      - >-
      ...
      - >-
        Put the device on a strong Wi-Fi connection and leave it awake. Very large
        PDFs can take several minutes to arrive.
      - >-
  ...
  - title: Clear a stuck sync, and understand subscription limits
    ...
    steps:
      - >-
        Restart the Paper Pro from Settings, then "General", then "Power". Restart
