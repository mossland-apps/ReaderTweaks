---
title: The BOOX Note Air 4 C won't turn on or is frozen
...
symptoms:
  ...
  - The screen is frozen and touch and pen do nothing.
...
mostCommonCause: >-
  ...
  most common is an Android hang, which a long power-button hold clears. A bad
  app update can also cause a boot loop.
decisionTree:
...
fixes:
  - title: Charge it, then force a restart
    rationale: >-
      ...
      charge, and the screen stays dark the whole time.
    steps:
      - Plug the device into a wall charger with a known-good USB-C cable. Leave it for at least 30 minutes, longer if you can.
      - With it still plugged in, press and hold the power button for about 20 to 30 seconds. The screen blanks and then shows the start-up logo.
      - Release and let it boot. After a full drain this can take a couple of minutes.
      - If nothing happens after an hour on a proper charger and a 30-second hold, try another cable and charger.
    note: >-
  ...
  - title: Force a restart, and use recovery for a boot loop
    ...
    steps:
      - Press and hold the power button for about 30 seconds until the logo appears, then release, and let it finish starting.
      - >-
        If it hangs on the logo repeatedly, hold the power button to turn it
        fully off, then hold power together with a volume key (as your model's
        manual specifies) to enter the recovery menu.
      - In recovery, choose "wipe cache partition" — this does not delete your apps, books or notebooks. Then "reboot system now".
      - If the loop continues after a cache wipe, the next option is a factory reset, which does erase the device — see recovery notes below.
  - title: Remove a bad app in safe mode
    rationale: >-
      ...
      cause.
    steps:
      - Restart, and as it boots hold the volume-down button to start Android in safe mode. If it is stable in safe mode, an app is the cause.
      - In safe mode, open Settings, then Apps, and uninstall the app you added or updated just before the trouble.
      - Restart normally. Reinstall the app later if you still want it, from a fresh download.
recovery: >-
  A forced restart and a cache wipe leave your apps, books and notebooks in
  place. A factory reset erases everything on the device — installed apps, their
  data, notebooks not synced to the BOOX cloud, and any books in internal
  storage — but not books on a microSD card, and not anything already synced to
  a cloud account. Export important notebooks as PDF and back up sideloaded books
  before a reset. If nothing revives the device, contact BOOX support.
related:
  - text: "Set up a new BOOX Note Air 4 C"
    href: /devices/boox/boox-note-air-4-c/guides/first-setup/
  - text: "The BOOX Note Air 4 C battery drains fast"
    href: /devices/boox/boox-note-air-4-c/troubleshooting/battery-drains-fast/
---
...
recent app needs safe mode to remove that app. Export notebooks to PDF before
ever doing a factory reset.
