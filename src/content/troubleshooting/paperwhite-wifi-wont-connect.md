---
title: Kindle Paperwhite won't connect to Wi-Fi
pageSlug: wifi-wont-connect
device: paperwhite
software: Kindle firmware
summary: >-
  The Paperwhite will not join your network, drops the connection, or connects
  but cannot download books. Most cases are the router, the password, or a
  Kindle that needs a restart.
difficulty: Easy
...
mostCommonCause: >-
  On a modern Paperwhite the most common cause is the network itself rather than
  the Kindle — usually a 5 GHz-only network, a mistyped password, or a router
  that has stopped handing out addresses and needs a restart. A close second is a
  Kindle that has been awake for a long time and needs a restart of its own.
decisionTree:
...
fixes:
  ...
  - title: Rule out the password and the router
    ...
    steps:
      ...
      - >-
        On the Kindle, tap your network name, choose "Forget", then connect to
  ...
  - title: Restart both devices, then check downloads
    ...
    steps:
      ...
      - >-
        ...
        wrong.
      - >-
        On a hotel or office network that shows a sign-in page, the Kindle's
        basic browser often cannot load it. Use your phone to sign in to that
        network, then connect the Kindle while the phone keeps the session
        alive, or use a phone hotspot instead.
recovery: >-
  None of these steps remove content. If you deregistered the Kindle and cannot
  get it to register again, connect it to a computer by USB to keep reading books
  that are already on it, and try registration later from Settings, then Your
  Account. A full "Reset" from Device Options is a last resort and erases
  downloaded books, though not your Amazon purchases, which can be downloaded
  again.
related:
  - text: "Books bought or sent aren't showing up on the Kindle Paperwhite"
    href: /devices/kindle/paperwhite/troubleshooting/books-not-showing/
  - text: "Set up a new Kindle Paperwhite"
    href: /devices/kindle/paperwhite/guides/first-setup/
---

Work through the fixes in order unless the decision tree points you to a
specific one. Fix 1 and Fix 2 together resolve almost every "will not connect"
case on a home network; Fix 3 is for connections that form but do not hold.
