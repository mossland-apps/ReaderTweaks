---
title: A DRM-protected library book won't open on the PocketBook Era
...
summary: >-
  ...
  authorisation, or opens as an ".acsm" file that does nothing. It needs the Era
  signed in to an Adobe ID.
difficulty: Moderate
...
symptoms:
  ...
  - A library book opened on another device but not on the Era.
beforeYouStart:
...
mostCommonCause: >-
  The Era can open Adobe-DRM library books, but only once it is authorised with
...
decisionTree:
  ...
  branches:
    ...
    - label: "Adobe sign-in itself fails with an activation limit"
      ...
      href: "#fix-2"
fixes:
  - title: Authorise the Era with an Adobe ID
    rationale: >-
    ...
    steps:
      - >-
        On the Era, open Settings, then "Accounts and Synchronization", then
        "Adobe DRM".
      - >-
    ...
    note: >-
      ...
      any computer, and the Era — so books unlock on all of them.
  - title: Make sure the book was fulfilled to the right Adobe ID
    rationale: >-
      ...
      one now on the device, the Era cannot open it, and re-authorising to a new
      ID does not retroactively unlock older books.
    steps:
      - >-
      ...
      - >-
        Sign the Era in to that same Adobe ID (Settings, then Accounts and
  ...
  - title: Complete the download from the ".acsm" file on the device
    rationale: >-
      ...
      exchanges for the actual book. Copying the ".acsm" onto the Era and opening
      it there completes the download; copying a book fulfilled on a computer
      usually does not carry the licence.
    steps:
      - >-
        Download the ".acsm" file from the library website rather than using a
        computer app to open it.
      - >-
        Copy just the ".acsm" file to the Era by USB, into any folder.
      - >-
        On the device, open the ".acsm" from the Library. With Adobe DRM
        authorised (Fix 1), the Era contacts Adobe, downloads the real EPUB or
        PDF, and replaces the ".acsm" entry with the book.
      - >-
        If it still fails, the cleanest route is to fulfil the loan in the Libby
        app and use its "Send to device" option, which hands the Era a book
...
The Era is one of the few readers that opens Adobe-DRM library books without a
