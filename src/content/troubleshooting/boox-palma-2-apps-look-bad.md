---
title: Apps look grey, blurry or ghosted on the BOOX Palma 2
pageSlug: apps-look-bad
device: boox-palma-2
software: BOOX firmware
summary: >-
  Text looks washed out, faint traces of old screens linger, or scrolling
  smears. On the Palma 2 this is the refresh settings, not a fault.
difficulty: Easy
...
symptoms:
  ...
  - Faint images of previous screens stay visible.
  - Scrolling a feed or web page leaves a smeared trail.
...
mostCommonCause: >-
  The global refresh mode is left on a fast setting, or a phone-style app has no
  per-app optimisation set, so Android's normal fast rendering is shown through
  e-ink with no correction. Grey text is usually a fast mode plus no contrast
  boost; ghosting is a full-refresh frequency set too low.
decisionTree:
  ...
  branches:
    ...
    - label: One specific app looks bad, others are fine
      ...
      href: "#fix-2"
    - label: Old screens linger until a manual refresh
...
fixes:
  - title: Fix the global refresh mode
    ...
    steps:
      - Swipe down from the top for the control centre and find the refresh mode selector.
  ...
  - title: Optimise the specific app that looks bad
    rationale: >-
      Per-app settings override the global mode. A phone app with none set falls
      back to defaults that often look wrong on a small e-ink screen.
    steps:
      - Long-press the app in the BOOX app drawer and choose "Optimise", or use the control-centre optimisation icon while the app is open.
      - Set its refresh mode — HD or Regal for text, Balanced or Fast for a browser or feed reader.
      - If text is grey, turn on "enhance contrast" or "bleach whites" a little — not to the maximum.
      - If the app's layout is cramped, lower the DPI slider a notch, then reopen the app.
      - Turn on "animation filter" to stop fades and slide effects that smear.
...
related:
  ...
  - text: "Optimise a single app on the BOOX Palma 2"
    href: /devices/boox/boox-palma-2/customization/optimise-an-app/
---
